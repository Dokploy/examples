# Laravel 11 Application Deployment with Nixpacks and PostgreSQL

This guide provides instructions for deploying a fresh Laravel 11 application (breeze, with vite + nginx as webserver) using Nixpacks. It does not host a database server itselft, but is configured to use an external PostgreSQL database server, which can also be hosted in dokploy / coolify.

## Usage

Copy the `nixpacks.toml` file to your project root. Feel free to modify it to your needs. Then configure your deployment-application (dokploy) to use [Nixpacks as build type](https://docs.dokploy.com/en/docs/core/application/build-type) option.
